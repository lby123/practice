...
void CInstantAIDlg::InitListView()
{
   ...
   int j = 0;
	...
	{
		pcolorList->SetItemText(ROW_OF_CH(ch) / LV_COL_COUNT, COL_OF_CH(ch), str);//初始0.0000
	}
}
...
void CInstantAIDlg::OnTimer(UINT_PTR nIDEvent)
{
   ...
   DOUBLE data[128];
	errorCode =  m_instantAiCtrl->Read(m_confParam.channelStart, m_confParam.channelCount, data);
	CheckError(errorCode);
   ...
   int dataCyclic = GetDlgItemInt(IDC_EDIT_INTERVAL);//取输入的数值
	m_simpleGraph.Chart(data, m_confParam.channelCount, 1,1.0*dataCyclic/1000);//划线
...
void CInstantAIDlg::RefreshListView(DOUBLE data[])
{
	...
	for (int ch = 0; ch < m_confParam.channelCount; ch++)
	{
      str.Format(_T("%.6g"),data[ch]);
		pcolorList->SetItemText(ROW_OF_CH(ch) / LV_COL_COUNT, COL_OF_CH(ch), str);//出数字

		if((fp=fopen("c:test.txt","a"))==NULL)
...
void CInstantAIDlg::OnBnClickedButtonStart()
{
	...
	//int dataCyclic = pslider->GetPos();
   //CString str ;
    //str.Format(_T("%d"),dataCyclic); 显示滑块数值

	int dataCyclic = GetDlgItemInt(IDC_EDIT_INTERVAL);
